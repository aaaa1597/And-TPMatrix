...
void Matrix::PerspectiveMatrix2(float viewpoint, const float lhsm[4][4], float retmat[4][4]) {
	...
	Matrix::MultiMatrix((const float(*)[4])perspectiveM, lhsm, retmat);
	return;
}
...
//		__android_log_print(ANDROID_LOG_DEBUG, "aaaaa", "aaaaa pDstVertex[%d].y(%d)=pSrcVertex[%d].x(%d)*mat[0][1](%f) + pSrcVertex[%d].y(%d)*mat[1][1](%f) + mat[3][1](%f)", lpct, pDstVertex[lpct].y, lpct,pSrcVertex[lpct].x, mat[0][1], lpct,pSrcVertex[lpct].y, mat[1][1],mat[3][1]);
        pDstVertex[lpct].x = pSrcVertex[lpct].x*mat[0][0] + pSrcVertex[lpct].y*mat[1][0] + mat[3][0];
        pDstVertex[lpct].y = pSrcVertex[lpct].x*mat[0][1] + pSrcVertex[lpct].y*mat[1][1] + mat[3][1];
	}
	return;
}
...
void Matrix::ConvVertexf3(int nPoint, const GPointf3 *pSrcVertex, const float mat[4][4], GPointf3 *pDstVertex) {
	for(size_t lpct = 0; lpct < nPoint; lpct++) {
        pDstVertex[lpct].x = pSrcVertex[lpct].x*mat[0][0] + pSrcVertex[lpct].y*mat[1][0] + pSrcVertex[lpct].z*mat[2][0] + mat[3][0];
        pDstVertex[lpct].y = pSrcVertex[lpct].x*mat[0][1] + pSrcVertex[lpct].y*mat[1][1] + pSrcVertex[lpct].z*mat[2][1] + mat[3][1];
        pDstVertex[lpct].z = pSrcVertex[lpct].x*mat[0][2] + pSrcVertex[lpct].y*mat[1][2] + pSrcVertex[lpct].z*mat[2][2] + mat[3][2];

        float w = pSrcVertex[lpct].x*mat[0][3] + pSrcVertex[lpct].y*mat[1][3] + pSrcVertex[lpct].z*mat[2][3] + mat[3][3];
